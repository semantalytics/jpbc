...
public class UTMAStrongParametersGenerator {
    ...
    public void store(OutputStream outputStream, UTMAStrongParameters parameters) throws IOException {
        ...
        // Store masterSecretKeyParameters
    ...
    public UTMAStrongParameters load(InputStream inputStream) throws IOException {
        ...
        // load masterSecretKeyParameters
        Element t1 = pairing.getZr().newElement(); t1.setFromBytes(readByteArray(in));
        Element t2 = pairing.getZr().newElement(); t2.setFromBytes(readByteArray(in));
        Element t3 = pairing.getZr().newElement(); t3.setFromBytes(readByteArray(in));
        Element omega = pairing.getZr().newElement(); omega.setFromBytes(readByteArray(in));
        ...
        return new UTMAStrongParameters(
                utmaPublicParameters,
                new UTMAStrongRPublicParameters(rPrivateKey),
                new UTMAStrongMasterSecretKeyParameters(utmaPublicParameters,
                                                        t1.getImmutable(), t2.getImmutable(), t3.getImmutable(),
                                                        omega.getImmutable())
        );
