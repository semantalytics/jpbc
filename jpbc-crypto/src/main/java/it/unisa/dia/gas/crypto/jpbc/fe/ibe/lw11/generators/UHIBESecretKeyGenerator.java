 ...
 */
public class UHIBESecretKeyGenerator implements CipherParametersGenerator {
    ...
    public CipherParameters generateKey() {
        if (params instanceof UHIBESecretKeyGenerationParameters) {
            UHIBESecretKeyGenerationParameters parameters = (UHIBESecretKeyGenerationParameters) params;

            UHIBEPublicKeyParameters pk = parameters.getPublicKey();
            UHIBEMasterSecretKeyParameters msk = parameters.getMasterSecretKey();
            ...
            return new UHIBESecretKeyParameters(pk.getCurveParams(),
                                                K0s, K1s, K2s, K3s,
                                                parameters.getIds());
        } else if (params instanceof UHIBEDelegateSecretKeyGenerationParameters) {
            UHIBEDelegateSecretKeyGenerationParameters parameters = (UHIBEDelegateSecretKeyGenerationParameters) params;

            UHIBEPublicKeyParameters pk = parameters.getPublicKey();
            UHIBESecretKeyParameters sk = parameters.getSecretKey();
            Pairing pairing = PairingFactory.getPairing(pk.getCurveParams());
        ...
        throw new IllegalStateException("UHIBESecretKeyGenerator not initialized correctly.");
    }
}
